    ...
    if ((CurrentPointNum > 0) || (CurrentNegPointNum > 0))
    {
        ...
        if (AW_Base.PeakCheckFrameCnt == 5)			// 5 cycles trigger one shot
        {
            ...
            if ((AW_Base.PosPeakCnt == 0) && (AW_Base.NegPeakCnt == 0))
            {
				...
				//printk("CNT = %d\n", CurrentPointNum+CurrentNegPointNum);
            ...
            else 
            {
				...
				//printk("AW_Base.PosPeakCnt = %d, AW_Base.NegPeakCnt = %d\n",AW_Base.PosPeakCnt,AW_Base.NegPeakCnt);
                for (i = 0; i < (AW_Base.PosPeakCnt + AW_Base.NegPeakCnt); i++)		// check if it is long touch
                {
                    ...
                    else
                    {
						...
						//printk("peakStruct = NEGPeak %d,%d,%d\n",pstructPeak->PeakY,pstructPeak->PeakX,pstructPeak->LongTouchValid );
                    }
                    ...
                    else
	        	    {
						...
						if (pstructPeak->PeakY < ChkPointsNum)
						{
							RowMin = 0;
							RowMax = pstructPeak->PeakY + ChkPointsNum + (ChkPointsNum - pstructPeak->PeakY);
							//RowMax = ChkPointsNum<<1;
						}
						else if (pstructPeak->PeakY > (AWTPCfg.TX_LOCAL-ChkPointsNum-1))
						{
							RowMin = pstructPeak->PeakY - ChkPointsNum - (ChkPointsNum+1 - AWTPCfg.TX_LOCAL + pstructPeak->PeakY);
							RowMax = AWTPCfg.TX_LOCAL;
						}
						else
						{
							RowMin = pstructPeak->PeakY - ChkPointsNum;
							RowMax = pstructPeak->PeakY + ChkPointsNum;
						}
                        //for(m = 0; m < AWTPCfg.TX_LOCAL; m ++)	
						for(m = RowMin; m <= RowMax; m ++)		// Modified by YaoWei 2012.7.26
                        {
                            //The same point 
						...
						// Added 2012.8.7
						DiffTemp = 0xfff;
						if (pstructPeak->PeakY > AWTPCfg.TX_LOCAL>>1)
						{
							for (m = pstructPeak->PeakY-1; m > pstructPeak->PeakY-6; m--)
							{
								if (ABS(DiffTemp) > ABS(Diff[m][pstructPeak->PeakX]))
								{
									DiffTemp = Diff[m][pstructPeak->PeakX];
									RowMin = m;
									if (ABS(DiffTemp) < 10)
									{
										break;
						...
						DiffTemp2 = temp - AW_Base.Base[RowMin][pstructPeak->PeakX]; 
						  	
		        			if (DiffTemp2 > THPEAK*2)
	            				{
						...
						*/
                        ...
                        //if (n >= (AWTPCfg.TX_LOCAL>>1))

						//printk("N = %d\n",n);
						if (n >= ChkPointsNum)		// Modified by YaoWei 2012.7.26
                        {
