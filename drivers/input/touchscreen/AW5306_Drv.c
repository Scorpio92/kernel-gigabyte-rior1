...
#if 1
#define	BIGAREA_PEAK_TH				90
#define	THPEAK						70	//suspected touch threshold
#define	THGROUP						80	// touch threshold
#else
#define	BIGAREA_PEAK_TH				140
#define	THPEAK						120		//suspected touch threshold
#define	THGROUP						130		// touch threshold
#endif
...
char AW5306_WorkMode = RawDataMode;//DeltaMode;//RawDataMode;
...
#define CALI_FILENAME	"/productinfo/tpcali"
...
void TP_Calibration(void)
{
	...
	{
		ret = nvram_read(CALI_FILENAME,&buf[0],100,0);
...
void InitDeltaMode(void)
{
	...
	ret = I2C_WriteByte(SA_SCANMD,0x87);	//DIFF MODE  start scan
}
...
void InitMonitorMode(void)
{
	...
	ret = I2C_WriteByte(SA_SCANMD,0x81);	//DIFF MODE 
}
...
{
	unsigned char i,j,intstat,readNum;
	short captemp;
	unsigned char tx,rx;
	unsigned char data[504];
...
{
	unsigned char i,j,p_k,P_N,p_i,NegCnt,i_mid,j_mid;
	unsigned char j_temp=0;
	unsigned char i_temp=0;
	short Data,BigAreaCnt,t_left,t_right,t_up,t_down;
	short MinValue,Temp;
	...
	for(i=0;i<AWTPCfg.TX_LOCAL;i++)
	{
		for(j=0;j<AWTPCfg.RX_LOCAL;j++)
		{
			//Data = ABS(Diff[i][j]);
			Data = Diff[i][j];
			if((Data > BIGAREA_PEAK_TH) || (Data < -BIGAREA_PEAK_TH) )
			{
				BigAreaCnt++;
			}
			if((Data > THPEAK) /*|| (Diff[i][j] < WATER_NEG_TH)*/)
			{
				if(i == 0)				
				{
					t_left = 0;
				...
				if(Data >= t_left)			//Possible Peak Point
				{
				#if 0
					if(Diff[i][j] < 0)			//Neg Peak
					{
						i_temp = i*2;
						j_temp = j*2;
						if(NegCnt == 0)
	...
	AW_Base.BigAreaTouchFlag = 0;

	if(BigAreaCnt >= BIGAREA_PEAK_NUM)
	{
		AW_Base.BigAreaTouchFlag = 1;
	}
#ifdef WATER_PROOF
	WaterDefense();
#endif
...
void ESD_Process(void)
{
	...
	{
	    ucFiterCnt += ESD_FILTER_FRAMES * (cnt - (AWTPCfg.RX_LOCAL - 4));
	}
	...
	if(AW_Peak.CurrentPointNum > AW_Peak.LastPointNum)
	{
		if(TouchedCnter < AW_Frame.FilterPointCnt)

		{
			TouchedCnter++;
			AW_Peak.CurrentPointNum = AW_Peak.LastPointNum;
		}
	}
	else
	{
...
void CalculateCoordinate(void)
{
	...
	if(AW_Peak.CurrentPointNum != 0)
	{
				...
				for(j = startY; j <= EndY; j++)
				{
					if((i>=0) && (i<AWTPCfg.TX_LOCAL-AWTPCfg.HAVE_KEY_LINE) && (j>=0) && (j<AWTPCfg.RX_LOCAL))
					{
						...
						if(TouchDataDiff > 0)
						{
							...
							{						
								sum_X[k] += j*TouchDataDiff;
								sum_tb[k] += TouchDataDiff;
								sum_Y[k] += i*TouchDataDiff;
								sum_t[k] += TouchDataDiff;
							}
							else																					//外圈，需要判断此点与其它peak点之间的距离
							{
								for(i_mid = 0; i_mid < AW_Peak.CurrentPointNum;i_mid++)
								{
									if(i_mid != k)
									{
										if((CalDiff(i_temp, AW_Peak.Peak[i_mid][0]) < 5) && (CalDiff(j_temp, AW_Peak.Peak[i_mid][1]) < 5))		//有一个小于5，则退出 for 循环， 
										{
											i_mid = 0xFF;
											break;
										}
		...
		for(k=0;k<AW_Peak.CurrentPointNum;k++)
		{
			...
			{
				sum_X[k] = sum_X[k]*64;
				t_left = (short)(sum_X[k]/sum_tb[k]) + 32;

				sum_Y[k] = sum_Y[k]*64;
				t_up = (short)(sum_Y[k]/sum_t[k]) + 32;
...
void preFilterTouchPoint(void)
{
	...
	unsigned char CurrentPointNum = AW_Frame.PointNum;
	unsigned char LastPointNum = AW_Frame.LastPointNum;
	...
	else
	{
		...
		for (i = 0; i < CurrentPointNum; i++)
		{
			if (AW_LastPoint[AW_Frame.PointInfo[i].PointID].X != 0x7fff)		// This ID has exist in last frame
			{
		...
		for (i = 0; i < CurrentPointNum; i++)
		{
			if (AW_LastPoint[AW_Frame.PointInfo[i].PointID].X == 0x7fff)		// No match ID found
			{
				AW_Frame.RptPoint[j].X = AW_Frame.PointInfo[i].X;
				AW_Frame.RptPoint[j].Y = AW_Frame.PointInfo[i].Y;
				AW_Frame.RptPoint[j].PointID = AW_Frame.PointInfo[i].PointID;
		...
		for (k = 0; k<CurrentPointNum; k++)
		{
			//CurrentFrame.RptPoint[k].X <<=  4;
			//CurrentFrame.RptPoint[k].Y <<=  4;
			if (AW_LastPoint[AW_Frame.RptPoint[k].PointID].X != 0x7fff)
			{
				TouchDiff = CalDiff(AW_Frame.RptPoint[k].X, AW_LastPoint[AW_Frame.RptPoint[k].PointID].X)
					+ CalDiff(AW_Frame.RptPoint[k].Y, AW_LastPoint[AW_Frame.RptPoint[k].PointID].Y);
				TouchDiff <<= 4;
		
				if (TouchDiff < (THDIFF >>3))		// 320  --> 80
				{
					temp = 4;		// 16
				}
				else if (TouchDiff < (THDIFF >>2))		// 640  --> 160
				{
        			temp = 3;		// 8
				}
				else if (TouchDiff < (THDIFF >>1))		// 1280  --> 320
				{
        			temp = 2;		// 4
				}
				else if (TouchDiff < THDIFF)		// 2560  --> 640
				{
					temp = 1;   		// 2
				}
				else
				...
				TouchDelta = (signed short)AW_Frame.RptPoint[k].X - (signed short)AW_LastPoint[AW_Frame.RptPoint[k].PointID].X;
				TouchDelta -= TouchDelta / (1 << temp);  // 确保算术右移
				if ((temp != 0) && (TouchDelta != 0))
				{
					AW_Frame.RptPoint[k].X -= TouchDelta;
				}
				TouchDelta = (signed short)AW_Frame.RptPoint[k].Y - (signed short)AW_LastPoint[AW_Frame.RptPoint[k].PointID].Y;
				TouchDelta -= TouchDelta / (1 << temp);
				if ((temp != 0) && (TouchDelta != 0))
				{
	...
	//CurrentFrame.RptPoint[0].X = CurrentFrame.RptPoint[0].X >> 4;
	//CurrentFrame.RptPoint[0].Y = CurrentFrame.RptPoint[0].Y >> 4;
		...
		else 
		{
			if (AW_Frame.PointNum <= 3) 
			{
				...
				{  
					...
					for(i = 0; i < AW_Frame.PointNum;i ++) 
					{
						...
						if (AW_LastPoint[CurrentIDList[i]].X == 0x7FFF) // 如果上一帧不存在该ID（IDList[i]）则Delta = 0x3ff_ffff
						{  
						...
						else 
						{
							...
							Delta_tmp1 = CalDiff(AW_Frame.PointInfo[i].X,AW_LastPoint[CurrentIDList[i]].X);
							Delta_tmp2 = CalDiff(AW_Frame.PointInfo[i].Y,AW_LastPoint[CurrentIDList[i]].Y);
			...
			{	
				for(i=0;i<AW_Frame.PointNum;i++) 
				{
					...
					{	// 寻找上一帧中与该点距离最近的点
						Delta_tmp1 = CalDiff(AW_Frame.PointInfo[i].X,AW_LastPoint[j].X)+\
								 CalDiff(AW_Frame.PointInfo[i].Y,AW_LastPoint[j].Y);
...
#ifdef FLY_BROKEN_LINE_METHORD
void LiftUpProcess(void)
{
	unsigned char i,j,k,PointNum,PrePointNum;
	short Distance;
	unsigned char PointID;
	unsigned char IdUsed [MAX_POINT];
	unsigned char IdIndex [MAX_POINT];
	STRUCTPOINT 	CurPoint[MAX_POINT];
	char FlyFlag;

	PointNum = AW_Peak.CurrentPointNum;
	PrePointNum = AW_Peak.LastPointNum;
	...
	for(i =0; i<MAX_POINT; i++)
	{
		...
		CurPoint[i].X = 0x7FFF;
		CurPoint[i].Y = 0x7FFF;
		CurPoint[i].PointID = 0xFF;
		if(0xFF == AW_LastPoint[i].PointID)
		{
			pointDistance[i] = 0;
			flycnt[i] = 2;
		}
	...
	for(i =0; i<MAX_POINT; i++)
	{
		...
		if ((IdUsed[i] == 0) && (AW_LastPoint[i].PointID == 0xFF))		// ID using: current no; pre no
		{
			AW_LastPoint[i].X = 0x7FFF;
			AW_LastPoint[i].Y = 0x7FFF;
			AW_LastPoint[i].PointID = 0xFF;
		...
		else if ((IdUsed[i] == 0) && (AW_LastPoint[i].PointID < MAX_POINT))		// ID using: current no; pre yes
		{
			if (AW_Frame.PointHoldCnt[i] >= HOLD_TURN)		// Need Holding to avoid line broken
			{
				CurPoint[j].X = AW_LastPoint[i].X;
				CurPoint[j].Y = AW_LastPoint[i].Y;
				if (AW_Frame.PointHoldCnt[i] == HOLD_TURN)
				{
					CurPoint[j].Event = UP_EVENT;
				}
				else
				{
					CurPoint[j].Event = CONTACT_EVENT;
				}
				CurPoint[j].PointID = AW_LastPoint[i].PointID ;
				AW_Frame.PointHoldCnt[i]--;	
				j++;
				k++;
			}
			else	// Current Frame: not report, Last Frame: not report
			{
				AW_Frame.PointHoldCnt[i] = 0;
				AW_LastPoint[i].X = 0x7fff;
				AW_LastPoint[i].Y = 0x7fff;
		...
		else if ((IdUsed[i] == 1) && (AW_LastPoint[i].PointID == 0xff))	// ID using: current YES; pre NO
		{
			AW_Frame.PointHoldCnt[i] = 1;
			AW_LastPoint[i].X = AW_Frame.RptPoint[IdIndex[i]].X;
			AW_LastPoint[i].Y = AW_Frame.RptPoint[IdIndex[i]].Y;
			AW_LastPoint[i].PointID = i;
			k++;
		}
		else if ((IdUsed[i] == 1) && (AW_LastPoint[i].PointID < MAX_POINT))	// ID using: current YES; pre YES
		{
			Distance = CalDiff(AW_Frame.RptPoint[IdIndex[i]].X, AW_LastPoint[i].X) + CalDiff(AW_Frame.RptPoint[IdIndex[i]].Y, AW_LastPoint[i].Y);
			if (AW_Frame.PointHoldCnt[i] >= HOLD_TURN)		// Need Holding to avoid line broken
			{
				if (AW_Frame.PointHoldCnt[i] == HOLD_TURN)
				{
					CurPoint[j].Event = DOWN_EVENT;
				}
				else
				{
					CurPoint[j].Event = CONTACT_EVENT;
				...
				if ((Distance < AWTPCfg.FLYING_TH)||(FlyFlag == 0))
				{
					CurPoint[j].X = AW_Frame.RptPoint[IdIndex[i]].X;
					CurPoint[j].Y = AW_Frame.RptPoint[IdIndex[i]].Y;
					CurPoint[j].PointID = i;
					j++;

					AW_Frame.PointHoldCnt[i] = HOLD_MAX;
					AW_LastPoint[i].X = AW_Frame.RptPoint[IdIndex[i]].X;
					AW_LastPoint[i].Y = AW_Frame.RptPoint[IdIndex[i]].Y;
					AW_LastPoint[i].PointID = i;
					k++;
				...
				else
				{
					...
					//AW_LastPoint[i].X = AW_Frame.RptPoint[IdIndex[i]].X;	//AW_LastPoint Keep no change
					//AW_LastPoint[i].Y = AW_Frame.RptPoint[IdIndex[i]].Y;
					//AW_LastPoint[i].PointID = i;
				}
			}
			else
			{
				...
				if (Distance < AWTPCfg.FLYING_TH)
				{
					AW_Frame.PointHoldCnt[i]++;
					//CurPoint[j].X = AW_Frame.RptPoint[IdIndex[i]].X;
					//CurPoint[j].Y = AW_Frame.RptPoint[IdIndex[i]].Y;
					//CurPoint[j].Event = CONTACT;
					//CurPoint[j].PointID = i;
					//j++;
	...
	for(i=0;i<MAX_POINT;i++)
	{
		AW_Frame.RptPoint[i].X = CurPoint[i].X;
		AW_Frame.RptPoint[i].Y = CurPoint[i].Y;
		AW_Frame.RptPoint[i].PointID = CurPoint[i].PointID;
	}
	AW_Frame.PointNum = j;
	AW_Frame.LastPointNum = k;
	
}

#endif

void LiftUpFilter(void)
{
	...
	else if(AW_Frame.PointNum < AW_Frame.LastPointNum)		//current point < last point  should check to avoid broke line
	{
		...
		for(i=0;i<MAX_POINT;i++)
		{
			if ((IdUsed[i] == 0) && (AW_LastPoint[i].PointID < MAX_POINT))		// ID using: current no; pre yes
			{
				if (AW_Frame.PointHoldCnt[i] < HOLD_TURN)
				{
					AW_Frame.PointHoldCnt[i]++;
					AW_Frame.RptPoint[AW_Frame.PointNum].X = AW_LastPoint[i].X;
					AW_Frame.RptPoint[AW_Frame.PointNum].Y = AW_LastPoint[i].Y;
					AW_Frame.RptPoint[AW_Frame.PointNum].PointID = AW_LastPoint[i].PointID;
					AW_Frame.PointNum ++;
					if(AWTPCfg.DEBUG_SWITCH == 1)
					{
						printk("BROKEN LINE TRIGER!!!!!! PointNum=%d, i=%d\n", AW_Frame.PointNum,i);
...
void flying_process(void)
{
	...
	{
		pointID = AW_Frame.RptPoint[i].PointID;
		if(AW_LastPoint[pointID].X != 0x7FFF)
		{
		...
		{	
			...
			AW_Frame.RptPoint[i].X = AW_Frame.RptPoint[i+1].X;
			AW_Frame.RptPoint[i].Y = AW_Frame.RptPoint[i+1].Y;
			AW_Frame.RptPoint[i].PointID = AW_Frame.RptPoint[i+1].PointID;
			AW_Frame.PointNum--;
			i--;
...
void PointOutput(void)
{
	...
	//copy current point to last point in ID ORDER
	for (i=0; i<AW_Frame.PointNum; i++)
	{
		pointID = AW_Frame.RptPoint[i].PointID;
		if(pointID != 0xFF)
