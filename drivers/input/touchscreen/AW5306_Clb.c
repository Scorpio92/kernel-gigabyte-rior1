...
unsigned char TxOffsetClbEn = 1;
int DiffBuff[NUM_TX][NUM_RX];
...
void AW5306_CAC_CLB(void)
{
	...
	if (AveScreen == 0)
	{
		...
		for (Col = 0; Col < AWTPCfg.RX_LOCAL; Col++)
		{
			...
			//{
			//	I2C_WriteByte(SA_RXCAC0+AWTPCfg.RX_LOCAL-1-Col+RX_START, RxCac[Col]);
			//}
			//else
			{
...
void AW5306_OFFSET_CLB(void)
{
	...
	for (i = 0; i < AWTPCfg.TX_LOCAL-AWTPCfg.HAVE_KEY_LINE; i++)
	{
		...
		{
			printk("OFFSET LINE TX %d,aVe = %d, flag = %d\n", i,Ave,flag[i]);
			ClbErrorCode |= 0x2;
		}
	}
	for (i = 0; i < AWTPCfg.RX_LOCAL; i++)
	{
		...
		if ((Ave <= AWTPCfg.OffsetClbExpectedMin-500) || (Ave >= AWTPCfg.OffsetClbExpectedMax+500 ))
		{
			printk("OFFSET LINE RX %d,ave=%d, flag = %d\n", i,Ave,flag[i]);
			ClbErrorCode |= 0x2;
		}
	}
}
