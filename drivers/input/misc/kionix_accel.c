...
#define KIONIX_ACCEL_WHO_AM_I_KXTJ2 0x09	//Andy add 20120824
...
#define KIONIX_ACCEL_GRP4	4	/* KXTJ2 */
		...
		if(atomic_read(&acceld->accel_enable_resume) > 0)
		{
			...
			if (err < 0) {
				...
				if(atomic_read(&acceld->accel_input_event) > 0) {
					...
					input_report_abs(acceld->input_dev, ABS_Y, (-1)*acceld->accel_data[acceld->axis_map_y]);
...
//	input_dev->dev.parent =&(acceld->input_dev->dev);// &acceld->client->dev;
	///*&client->dev.kobj*/&(acceld->input_dev->dev.kobj
}
...
static DEVICE_ATTR(enable, /*S_IRUGO|S_IWUSR*/S_IRUGO | S_IWUGO, kionix_accel_get_enable, kionix_accel_set_enable);
static DEVICE_ATTR(poll, /*S_IRUGO|S_IWUSR*/S_IRUGO | S_IWUGO, kionix_accel_get_delay, kionix_accel_set_delay);
...
static int __devinit kionix_verify(struct kionix_accel_driver *acceld)
{
	...
	switch (retval) {
		...
		case KIONIX_ACCEL_WHO_AM_I_KXTJ9:
			...
			break;
		case KIONIX_ACCEL_WHO_AM_I_KXTJ2:
			KMSGINF(&acceld->client->dev, "this accelerometer is a KXTJ2.\n");
			break;
		default:
			break;
	}
				 ...
				 const struct i2c_device_id *id)
{
	...
	switch (err) {
		...
		case KIONIX_ACCEL_WHO_AM_I_KXTIK:
		case KIONIX_ACCEL_WHO_AM_I_KXTJ9:
		case KIONIX_ACCEL_WHO_AM_I_KXTJ2:		//Andy added 20120824: I did not handle the direction issue. That's why I make it as group 4
	...
	err = sysfs_create_group(/*&client->dev.kobj*/&(acceld->input_dev->dev.kobj), &kionix_accel_attribute_group);
	if (err) {
