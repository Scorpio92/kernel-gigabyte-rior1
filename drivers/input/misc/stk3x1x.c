...
//shenwenjian add 
int __stk_3x1x_31xx;
EXPORT_SYMBOL_GPL(__stk_3x1x_31xx);
...
printk(KERN_INFO "high_thd %d low_thd%d\n",high_thd,low_thd);
}
                        ...
                        const struct i2c_device_id *id)
{
    ...
    if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C))
    {
     ...
     client->addr =0x48;
     err = stk3x1x_i2c_smbus_read_byte_data(client,STK_PDT_ID_REG);
      if(err != 0x13)
      {
        __stk_3x1x_31xx = 1;
        printk("%s  stk3x1x invalid id  err =%x \n",__func__,err);
        return -ENODEV;
      }
