...
#define RPR400_DRV_NAME	"apds990x-rpr400"
...
#define PS_ALS_SET_PS_TL          (10)	//Changed from (0x000) to avoid triggered by background noise. 
...
static int rpr400_set_enable(struct i2c_client *client, int enable)
{
	...
	if(enable > 0x0B)
	{
		printk(KERN_ERR "RPR400 enable error: invalid measurement time setting.\n");
		return -EINVAL;
	}
	else
	{
...
// Theorically it is not necesssary to do so, but I just want to avoid any potential error.  -- Andy 2012.6.6
	...
	//pr_debug(KERN_INFO "RPR400 als report: data0 = %d, data1 = %d, gain0 = %d, gain1 = %d, time = %d, lux = %d, level = %d.\n", als_ps->als_data0_raw, 
...
static void rpr400_ps_int_work_handler(struct work_struct *work)
{
	...
	if(tmp & PS_INT_MASK)	//Interrupt is caused by PS
	{
		...
		if(als_ps->ps_data > PS_ALS_SET_PS_TH)
		{		
			als_ps->ps_direction = 1;
		}
		else if(als_ps->ps_data < PS_ALS_SET_PS_TL)
		{
			als_ps->ps_direction = 0;
		}

	//	pr_debug(KERN_INFO "RPR400 ps report: raw_data = %d, data = %d, direction = %d. \n", 
...
static int rpr400_init_client(struct i2c_client *client)
{
	...
	//unsigned char  value= REG_SYSTEMCONTROL;
...
//	printk("%s interrupt is hooked\n", __func__);
	...
	if (err){
		printk(KERN_ERR "rpr400_init_client\n");
		goto exit_kfree;
	}


  /*sym read ID to distinguish apds from rpr4 */
   	value =  i2c_smbus_read_byte_data(client, REG_SYSTEMCONTROL);	//soft-reset
   	if(value == 9){
		als_rpr400 = 1;
	}else{
		als_rpr400 = 0;
...
*/
	...
	if (!als_ps->input_dev_ps) {
		...
		goto exit_free_dev_als;									//goto exit_free_dev_als;
	}
	...
	printk("%s End\n",__func__);
	printk(KERN_INFO "RPR400 probe: INT No. %d", client->irq);
	return 0;
...
static int __init rpr400_init(void)
{
	int ret = 0;
	printk("\n\nrpr400_init\n");
	ret =  i2c_add_driver(&rpr400_driver);
