...
/****************************************
History:
1.haijian.jin@ragentek BUG_ID:QELS-2906 jinhaijian 20121225
Description:light sensor not work fine
****************************************/
 /*lilonghui add it for debug on/off*/

/*lilonghui add it for the camera*/
...
#define APDS990x_DRV_NAME	"apds990x-rpr400"
#define DRIVER_VERSION		"1.0.4"

#define APDS990x_INT		81//IRQ_EINT20

#define APDS990x_PS_DETECTION_THRESHOLD		700 //600
#define APDS990x_PS_HSYTERESIS_THRESHOLD	 600
...
//add by hxh for fix bug vobis_266 2012.11.30 start
typedef enum 
{
...
//add by hxh for fix bug vobis_266 2012.11.30 end
static int read_pdata_flag = 0;
/*
add by hxh to fix the issue(2013.01.07):
if you set the lcdbacklight as manual,and the brightness is 255 in the darkness;then you change the lcdbacklight control as auto ,but the brightness do not change !
*/
static int first_report_als_value = 0;
/*
 * Global data
 */
...
static int LuxCalculation(struct i2c_client *client, int cdata, int irdata)
{
	...
	int IAC2=0;
	int IAC=0;
        //modify BUG_ID:QELS-2906 jinhaijian 20121225 begin
	int GA=1064;	//48 to 1064		/* 0.48 without glass window */
	int COE_B=210;	 //223 to 210	/* 2.23 without glass window */
	int COE_C=29;	//70 to 29	/* 0.70 without glass window */
	int COE_D=57;   //142 to 57		/* 1.42 without glass window */
        //modify BUG_ID:QELS-2906 jinhaijian 20121225 end
	int DF=52;
...
static void apds990x_change_ps_threshold(struct i2c_client *client)
{
	...
	if ( (data->ps_data > data->pilt) && (data->ps_data > data->piht) ) {
		...
		input_report_abs(data->input_dev_ps, ABS_DISTANCE,1);/* FAR-to-NEAR detection *///jinhaijian change 1 to 0
...
//		printk("far-to-near detected\n");
		...
		input_report_abs(data->input_dev_ps, ABS_DISTANCE,0);/* NEAR-to-FAR detection *///jinhaijian change 0 to 1
		input_sync(data->input_dev_ps);
...
static void apds990x_als_polling_work_handler(struct work_struct *work)
{
	...
	if(first_report_als_value ==1)
	 {
            luxValue++;
            first_report_als_value = 0;
	 }

	if(read_pdata_flag == 1)
		printk("pdata = %d \n",  pdata);
...
{
	struct apds990x_data *data = container_of((struct delayed_work *)work, struct apds990x_data, dwork);
	struct i2c_client *client=data->client;
	int	status;
	int cdata;
...
static irqreturn_t apds990x_interrupt(int vec, void *info)
{
	...
	struct apds990x_data *data = i2c_get_clientdata(client);
	disable_irq_nosync(APDS990x_INT);
	//printk("==> apds990x_interrupt2\n");
					...
					struct device_attribute *attr, const char *buf, size_t count)
{
	...
	//modifyed by hxh for fix bug vobis_266 2012.11.30 start
	/*
	...
	data->als_atime = apds990x_als_atime_tb[atime_index];
	//modifyed by hxh for fix bug vobis_266 2012.11.30 start
...
#else
static DEVICE_ATTR(als_poll_delay,  0644,
				    apds990x_show_als_poll_delay, apds990x_store_als_poll_delay);
#endif
...
static int apds990x_init_client(struct i2c_client *client)
{
	...
	err = apds990x_set_enable(client, 0);//0 modify by xiehui

	if (err < 0)
	{
		printk("%s fail to apds990x_set_enable\n", __func__);
		return err;
	}
	
	id = i2c_smbus_read_byte_data(client, CMD_BYTE|APDS990x_ID_REG);
	#ifdef CONFIG_AVAGO_APDS990X //sym
	if (id == 0x20) {
		printk("APDS-9901\n");
	}
	else if (id == 0x29) {
		...
		printk("Neither TMD-27711 nor TMD-27713\n");
		return -EIO;
	}
	 #endif
				   ...
				   const struct i2c_device_id *id)
{
	...
	if (request_irq(irq, apds990x_interrupt, IRQF_DISABLED|IRQ_TYPE_EDGE_FALLING,
		APDS990x_DRV_NAME, (void *)client)) {
		printk("%s Could not allocate APDS990x_INT !\n", __func__);
	
		goto exit_unregister_dev_ps;;
	}
	printk("sym apds990x probe end\n");
	return 0;
...
static int apds990x_resume(struct i2c_client *client)
{
	...
	//wake_lock(&ps_lock);
	enable_irq(APDS990x_INT);
	return apds990x_set_enable(client, 27);//0 modify by xiehui
...
/*cuirous problems for chip rpr400:
if first go  rpr400 ,next go apds990x; slave address 0x39 will be not found*/
static int __init apds990x_init(void)
{
	int ret = 0;
	printk("\n\napds990x_init\n ");
	if (als_rpr400 == 0)
		ret =  i2c_add_driver(&apds990x_driver);
