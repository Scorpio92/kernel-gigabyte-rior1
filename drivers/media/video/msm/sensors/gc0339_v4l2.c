						...
						uint16_t gain, uint32_t line)
{
  ...
  intg_time_msb = (unsigned int ) ((line & 0x0F00) >> 8);
  intg_time_lsb = (unsigned int ) (line& 0x00FF);
 CDBG("gc0339_write_prev_exp_gain,intg_time_msb=%d, intg_time_lsb=%d\n",intg_time_msb,intg_time_lsb);
  if(gain>0xff)
    gain=0xff;
  msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
    0x51,(gain),MSM_CAMERA_I2C_BYTE_DATA);
		...
		const struct i2c_device_id *id)
{
	...
	CDBG("%s IN\r\n", __func__);
/*renwei add it for the camera adaptive*/
#if 1
	if(is_rgtk_product(RGTK_PRODUCT_Q803)){
		gc0339_prev_settings[13].reg_data = 0x23;	
		gc0339_snap_settings[13].reg_data = 0x23;
		gc0339_recommend_settings[13].reg_data =0x23;
...
int32_t gc0339_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    printk("gc0339 readid ok, success\n");
	return rc;
}
