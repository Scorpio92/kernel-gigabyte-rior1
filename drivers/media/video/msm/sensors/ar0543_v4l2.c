...
static struct msm_camera_i2c_reg_conf ar0543_snap_settings[] = {
  ...
  {0x3004, 0x0008},/*x_addr_start*/
  {0x3008, 0x0A37},/*x_addr_end*/
  {0x3002, 0x0008},/*y_start_addr*/
  {0x3006, 0x079F},/*y_addr_end*/
  {0x3040, 0x0041},/*read_mode*/
  {0x034C, 0x0A20},//  {0x034C, 0x0A20},/*x_output_size*/
  {0x034E, 0x0798},//  {0x034E, 0x0798},/*y_output_size*/
...
static struct msm_sensor_output_info_t ar0543_output_settings[] = {
  { /* For SNAPSHOT */
    .x_output                = 0xA20,	//0xA20,    /* 2592 */
    .y_output                = 0x798,	//0x798,    /* 1944 */
  ...
  int update_type, int res)
{
  ...
  static uint16_t gain_value = 0x127;
  static uint16_t exposure_value = 0;
  s_ctrl->func_tbl->sensor_stop_stream(s_ctrl);
  if (update_type == MSM_SENSOR_REG_INIT) {
    ...
    if(res == 0)
      ...
      msleep(105);
    if(res == 0 ){
    CDBG("%s:lilonghui call we read the value before we take picture ",__func__);
    msm_camera_i2c_read(s_ctrl->sensor_i2c_client,0x305e,&gain_value, MSM_CAMERA_I2C_WORD_DATA);
    CDBG("%s:lilonghui read the gain_value(0x305e) =%d \n",__func__,gain_value);
    msm_camera_i2c_read(s_ctrl->sensor_i2c_client,0x3012,&exposure_value, MSM_CAMERA_I2C_WORD_DATA);
    CDBG("%s:lilonghui read the exposure_value(0x3012) =%d \n",__func__,exposure_value);
   }
    s_ctrl->func_tbl->sensor_group_hold_on(s_ctrl);
   ...
   if(res == 1){
      if(!csi_config)
      {
   CDBG("%s:lilonghui the first preview ,we will not write it ",__func__);
      }else{
   CDBG("%s:lilonghui we will write the value gain_value(0x305e) =%d exposure_value(0x3012) =%d\n ",__func__,gain_value,exposure_value);
   rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0x305E,(gain_value|0x1000),
    MSM_CAMERA_I2C_WORD_DATA);
   rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0x3012,exposure_value,
    MSM_CAMERA_I2C_WORD_DATA);
