...
static long mmc328x_ioctl(struct file *file,
...
static ssize_t mmc328x_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	    return sprintf(buf, "Chip : %s %s\nVersion : %s\n",
			                       "MEMSIC", "MMC328xMA", "4.0");
}

static DEVICE_ATTR(info, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, mmc328x_info_show, NULL);
...
static const struct attribute_group mmc328x_attr_group = {
	.attrs = mmc328x_attributes,
};


static ssize_t mmc328x_exist_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	    return sprintf(buf, "%d\n",exist);
}

static DEVICE_ATTR(mmc328x_exist, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, mmc328x_exist_show, NULL);


static struct attribute *mmc328x_exist_attributes[] = {
	&dev_attr_mmc328x_exist.attr,
	NULL
};

static const struct attribute_group mmc328x_attr2_group = {
	.attrs = mmc328x_exist_attributes,
};
...
static int mmc328x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	res = sysfs_create_group(&mmc328x_device.this_device->kobj, &mmc328x_attr_group);
	
#if 1
	res = sysfs_create_group(&client->dev.kobj, &mmc328x_attr2_group);
#endif
