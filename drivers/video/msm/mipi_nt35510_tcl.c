...
static struct dsi_cmd_desc nt35510_cmd_display_on_cmds[] = {
#if 0
	//{DTYPE_GEN_LWRITE, 1, 0, 0, 120, sizeof(exit_sleep), exit_sleep},
	...
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(cmd71), cmd71},  //
...
static int nt35510_read_panel_reg(struct msm_fb_data_type *mfd, char reg) 
{ 
...
unsigned int nt35510_monitor_reg_status(struct msm_fb_data_type *mfd) 
{ 
        uint32 data=0; 
        uint16 cnt = 0,ret=0; 
        uint8 err=0;     //ok 
       ...
       // if(mipi_dsi_wait_stop_state()) return 1; 
        ...
        ret = nt35510_read_panel_reg(mfd,0x0a); 
...
nt35510_monitor_reg_status(mfd); 
	return 0;
}
...
static void set_backlight_by_driver(struct msm_fb_data_type *mfd)
{
	...
	mipi_dsi_cmds_tx(mfd, &nt35510_tx_buf, tcl_cmd_drive_backlight_cmds,
...
static void  mipi_nt35510_tcl_set_backlight(struct msm_fb_data_type *mfd)
{
	if(driver_by_host) {
...
static struct msm_fb_panel_data nt35510_panel_data = {
	...
	.set_backlight = mipi_nt35510_tcl_set_backlight,
};

static int ch_used[3];

int mipi_nt35510_tcl_device_register(struct msm_panel_info *pinfo,
...
printk("luke: %s  %d \n", __func__, __LINE__	);
	...
	pdev = platform_device_alloc("mipi_nt35510_tcl_cmd_wvga_pt", (panel << 8)|channel);
