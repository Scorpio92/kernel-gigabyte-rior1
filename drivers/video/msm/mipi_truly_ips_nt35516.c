...
static char TCL_CUSTOMER_0X52[] = {0x53};  //write ctrl display
static struct dsi_cmd_desc nt35516_display_off_cmds[] = {
	{DTYPE_DCS_WRITE, 1, 0, 0, 30, sizeof(display_off), display_off},  //28
	{DTYPE_DCS_WRITE, 1, 0, 0, 30, sizeof(display_on), display_on},    //29
	{DTYPE_DCS_WRITE, 1, 0, 0, 30, sizeof(display_off), display_off},  //28
	{DTYPE_DCS_WRITE, 1, 0, 0, 30, sizeof(display_on), display_on},    //29
	{DTYPE_DCS_WRITE, 1, 0, 0, 30, sizeof(display_off), display_off},  //28
	{DTYPE_DCS_WRITE, 1, 0, 0, 120, sizeof(enter_sleep), enter_sleep}, //10
...
static struct dsi_cmd_desc nt35516_cmd_display_on_cmds[] = {
	...
	{DTYPE_DCS_WRITE, 1, 0, 0, 120,            sizeof(exit_sleep), exit_sleep},//11
	{DTYPE_DCS_WRITE, 1, 0, 0, 30, sizeof(display_on), display_on},    //29
	{DTYPE_DCS_WRITE, 1, 0, 0, 30, sizeof(display_off), display_off},  //28
	{DTYPE_DCS_WRITE, 1, 0, 0, 30, sizeof(display_on), display_on},    //29
	{DTYPE_DCS_WRITE, 1, 0, 0, 30, sizeof(display_off), display_off},  //28
...
unsigned int nt35516_monitor_reg_status(struct msm_fb_data_type *mfd) 
{ 
        uint32 data=0; 
        uint16 cnt = 0,ret=0; 
        uint8 err=0;     //ok 
...
static int mipi_nt35516_lcd_on(struct platform_device *pdev)
{
	...
	if (mfd->key != MFD_KEY)
		...
		}//add for continue_display_splash.hxh  20121109
...
printk("hxh: %s  %d \n", __func__, __LINE__	);
	...
	rc = gpio_direction_output(GPIO_TCL_LCD_RESET, 1);//modify 0->1 for continue_display_splash.hxh  20121109
	if (rc < 0) {
...
static void  mipi_nt35516_tcl_set_backlight(struct msm_fb_data_type *mfd)
{
	if(driver_by_host) {
...
static struct msm_fb_panel_data nt35516_panel_data = {
	...
	.set_backlight = mipi_nt35516_tcl_set_backlight,
};

static int ch_used[3];

int mipi_nt35516_truly_device_register(struct msm_panel_info *pinfo,
...
printk("hxh: %s  %d \n", __func__, __LINE__	);
	...
	pdev = platform_device_alloc("mipi_truly_nt35516_qhd_pt", (panel << 8)|channel);
...
static int __init mipi_nt35516_lcd_init(void)
{
...
module_init(mipi_nt35516_lcd_init);
