...
static int  mipi_truly_otm8018b_video_init(void)
{
	int ret;

#ifdef CONFIG_FB_MSM_MIPI_PANEL_DETECT
	if (msm_fb_detect_client("mipi_truly_otm8018b"))
		return 0;
#endif

	printk("mipi_truly_otm8018b_video_init ____start\n");
	pinfo.xres = 480;
	pinfo.yres = 800;
	...
	ret = mipi_truly_otm8018b_device_register(&pinfo, MIPI_DSI_PRIM,
...
static int __init mipi_truly_otm8018b_video_qhd_ips_init(void)
{
  mipi_truly_otm8018b_video_init();
  return 0;
}

module_init(mipi_truly_otm8018b_video_qhd_ips_init);
